...
from sqaapitest.src.utilities.genericutilities import generate_random_string, generate_random_coupon_code
from sqaapitest.src.helpers.coupons_helper import CouponsHelper
from sqaapitest.src.utilities.wooAPIUtility import WooAPIUtility
from sqaapitest.src.dao.coupons_dao import Coupons_DAO
import pytest
import random
...
def test_create_coupon_percent_discount_type(my_setup, discount_type):
    """
    Create a coupon with given 'discount type' verify the coupon is created.
    """

    LOG.info("Testing create coupon api for 50% off coupon.")
    ...
    coupon_dao = my_setup['coupon_dao']
    ...
    # verify coupon is actually created in database
    sql_rs = coupon_dao.get_coupon_by_id(coupon_id)
    assert coupon_id == sql_rs[0]['ID'], f"Created  coupon entry does not exists in DB"
...
def test_create_coupon_with_invalid_discount_type():
    ...
    LOG.info("Testing create coupon api for with invalid 'discount_type'.")
